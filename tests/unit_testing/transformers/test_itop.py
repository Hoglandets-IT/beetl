...
class UnitTestItopTransformers(unittest.TestCase):
    ...
    def test_orgcode__when_passed_differently_cased_values__casing_is_normalized_before_hashing(
        self,
    ):
        # arrange
        inFields = ["field1"]
        ...
        # act
        uppercaseData = DataFrame().with_columns(Series(inFields[0], [value.upper()]))
        uppercaseResult = transformer.orgcode(
            uppercaseData, inFields, outField, toplevel
        )
        uppercaseHash = uppercaseResult[outField][0]

        lowercaseData = DataFrame().with_columns(Series(inFields[0], [value.lower()]))
    ...
    def test_orgcode__when_comparing_hash_to_hash_from_same_value_plus_trailing_space__hashes_are_different(
        self,
    ):
        # arrange
        inFields = ["field1"]
        ...
        # act
        no_space_data = DataFrame().with_columns(Series(inFields[0], [value]))
        no_space_result = transformer.orgcode(
            no_space_data, inFields, outField, toplevel
        )
        no_space_hash = no_space_result[outField][0]

        with_space_data = DataFrame().with_columns(Series(inFields[0], [value + " "]))
