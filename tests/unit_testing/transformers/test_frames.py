...
class UnitTestFramesTransformers(TestCase):
    ...
    def test_coalesce_if__with_valid_input__out_field_is_selected_by_condition_with_none_value(
        self,
    ):
        data = DataFrame(
            [
                {"isNone": "not None", "primaryValue": "A", "fallbackValue": "B"},
                {"isNone": None, "primaryValue": "C", "fallbackValue": "D"},
            ]
        )
        config = {
            "conditionField": "isNone",
            "conditionValue": None,
            "trueField": "primaryValue",
            "falseField": "fallbackValue",
            "outField": "selectedValue",
