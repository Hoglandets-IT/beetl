...
## Coalesce if
Set a field to the value from one column if a condition matches, otherwise use the value from another column.
...
- transformer: frames.coalesce_if
  config:
    ...
    conditionValue: true
    trueField: primary_value
    falseField: fallback_value
    outField: selected_value
```
...
| is_active | primary_value | fallback_value |
|-----------|---------------|----------------|
| true | "Active User" | "Inactive User" |
| false | "Active User" | "Inactive User" |

**DataFrame after transformation**

| is_active | primary_value | fallback_value | selected_value |
|-----------|---------------|----------------|----------------|
| true | "Active User" | "Inactive User" | "Active User" |
| false | "Active User" | "Inactive User" | "Inactive User" |
...
- transformer: frames.coalesce_if
  config:
    conditionField: external_id
    conditionValue: null
    trueField: generated_external_id
    falseField: external_id
    outField: external_id
```