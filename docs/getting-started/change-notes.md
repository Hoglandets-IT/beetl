...
- The documentation has been updated with:
  ...
  - This [flowchart](/getting-started/change-notes.html) explaining the beetl sync process.
  - And a lot of other additions from data sources, transformers and examples.
...
### Breaking changes ⛓️‍💥
- The `source_configuration.columns` field has been deprecated and has been replaced by the mandatory `sync.comparisonColumns`. See the [documentation](/getting-started.html#configuration) and the section below about changes to sync columns.
...
- Previously the iTop data source queried a combination of fields specified as the columns in the old deprecated `source_configuration.columns` + the comparison fields specified in those columns `custom_options.itop.comparison_field`. Since this property has been replaced with the sync level `comparisonColumns` you now have to specify what fields you want to query, and what columns you want to skip while updating. See the [iTop documentation](/sources/itop.html).
- The dataset of items that will be created or updated at the destination will now have the schema of the tranformed source dataset since this is where they originate from. Likewise the dataset of items that are going to be deleted from the destination will have the schema of the transformed destination since they don't necessarily have the same schema as the source and can't be transformed into the same schema automatically. This differs from the last update where the delete dataset only included unique columns.
...
1. The schema to aid the comparing process has been moved from `sync.source_configuration.columns` and `sync.destination_configuration.columns` to `sync.comparisonColumns`. They have three valid properties `name`, `type` and `unique`, the last `custom_options` have been removed.
