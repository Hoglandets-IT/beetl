...
class StructTransformers(TransformerInterface):
    ...
    @staticmethod
    def list_columns_to_key_value_rows(data: pl.DataFrame, keyField: str, listField: str, objSource: str | None = None):
        """Convert list-valued DataFrame columns into key-value rows.

        Each processed column is treated as a key/list pair, where the column name is
        used as the key and the first row value is used as the list of values.

        For each value in the list, one row is created. The column name is written to
        keyField, and the list item is written to listField.

        If objSource is provided, only that column is processed. If objSource is None,
        all columns in the DataFrame are processed.

        Args:
            data (pl.DataFrame): The DataFrame to modify.
            keyField (str): Name of the output column that will contazin the source column name.
            listField (str): Name of the output column that will contain each list item.
            objSource (str | None): Optional name of one column to process.

        Returns:
            pl.DataFrame: A DataFrame with one row per processed column/list item pair.
        ...
        Example:
            ...
            `object_to_key_value_rows(data, "keyField", "listField")`
            ...
            `object_to_key_value_rows(data, "keyField", "listField", "field2")`
        ...
        if objSource is not None and objSource not in data.columns:
            raise ValueError(f"Column for {objSource} does not exist in dataframe")
        
        for source_obj in data:
            if objSource and source_obj.name != objSource:
                continue
            
            source_values = source_obj.to_list()            
            if not source_values:
                continue
            
            source_list = source_values[0]
            if not source_list:
                continue
            ...
            for source in source_list:
                new_obj  =  {keyField: source_obj.name,
                            listField: source}
